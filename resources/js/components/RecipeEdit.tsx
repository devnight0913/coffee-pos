...
class RecipeEdit extends Component<Props, State> {
    ...
    initializeData = () => {
        ...
        const recipe = JSON.parse(this.props.recipe);
        ...
        this.setState({ price: "" + recipe.price });
        this.setState({ status: recipe.is_active });
    
        const t_items = items.recipe_details.map((item: any, index: number) => {
            return {
        ...
        console.log(t_items);
        this.setState({ itemList: t_items });
    }
    ...
    saveRecipe = (): void => {
        ...
        httpService
            .post(`/recipe/${this.state.recipeId}`, {
            ...
            .then((response: any) => {
                if (response.data) {
                    // toast.info(t('Updated!', 'تم الحفظ'));
                    Swal.fire({
                        title: t('Updated', 'تم التحديث'),
                        text: t('Recipe has been updated!', '"تم تحديث الفاتورة!"'),
                        icon: 'success',
                                        ...
                                        <div className="mb-3">
                                            ...
                                            >
                                                ...
                                                .map((item: IItem, index: number) => (
                                                    <option key={index} value={item.name} selected={index === this.state.selectedCategory}>{item.name}</option>
                                                ))}
                                            </select>
                                        </div>
                                        <div className="mb-3">
...
const element = document.getElementById('recipe-edit');
if (element) {
    const props = Object.assign({}, element.dataset);
    const root = ReactDOM.createRoot(element);
    root.render(<RecipeEdit items={''} recipe={''} settings={''} {...props} />);
}
